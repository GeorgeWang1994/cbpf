...
type podMap struct {
	...
	Info  map[string]map[string]*k8s.K8sPodInfo
	mutex sync.RWMutex
}
...
func newPodMap() *podMap {
	return &podMap{
		Info:  make(map[string]map[string]*k8s.K8sPodInfo),
		mutex: sync.RWMutex{},
	}
}

func (m *podMap) add(info *k8s.K8sPodInfo) {
	m.mutex.Lock()
	podInfoMap, ok := m.Info[info.Namespace]
	if !ok {
		podInfoMap = make(map[string]*k8s.K8sPodInfo)
	}
...
func (m *podMap) get(namespace string, name string) (*k8s.K8sPodInfo, bool) {
	m.mutex.RLock()
...
func (m *podMap) getPodsMatchSelectors(namespace string, selectors map[string]string) []*k8s.K8sPodInfo {
	retPodInfoSlice := make([]*k8s.K8sPodInfo, 0)
	if len(selectors) == 0 {
