...
type mergableEvent struct {
	events []*model.Event // Keep No more than 10 events.
...
func newEvents(evt *model.Event) *events {
	return &events{
		event: evt,
	}
}
...
func (evts *events) getEvent(index int) *model.Event {
	if index == 0 {
		return evts.event
	}
...
func (evts *events) mergeEvent(evt *model.Event) {
	if evts.mergable == nil {
		firstEvt := evts.event
		evts.mergable = &mergableEvent{
			events:  []*model.Event{firstEvt},
...
func (evts *events) IsTimeout(newEvt *model.Event, timeout int) bool {
...
func (mps *messagePairs) mergeConnect(evt *model.Event) {
	mps.mutex.Lock()
	if mps.requests == nil {
...
func (mps *messagePairs) mergeRequest(evt *model.Event) {
	mps.mutex.Lock()
	if mps.requests == nil {
...
func (mps *messagePairs) mergeResponse(evt *model.Event) {
	mps.mutex.Lock()
...
func getMessagePairKey(evt *model.Event) messagePairKey {
	if evt.IsUdp() == 1 {
		return messagePairKey{
			pid:   evt.GetPid(),
			fd:    evt.GetFd(),
